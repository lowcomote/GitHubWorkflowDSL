...
public class GithubwfRootOnOneOf2Workflow_dispatchInputsPatternProperties0AllOf0IfValidator extends AbstractGithubwfRootOnOneOf2Workflow_dispatchInputsPatternProperties0AllOf0IfValidator {
	
	@Inject githubwf.githubwf.services.GithubwfRootOnOneOf2Workflow_dispatchInputsPatternProperties0AllOf0IfGrammarAccess grammarAccess;	
	
	private List<EObject> processedPatternProperties = new ArrayList<EObject>();

	@Check
	public void constValidationGithubwfRootOnOneOf2Workflow_dispatchInputsPatternProperties0AllOf0IfType(githubwf.GithubwfRootOnOneOf2Workflow_dispatchInputsPatternProperties0AllOf0IfType enclosingEClass){
		...
		EnclosingSchema enclosingSchema = relatedSchemas.getEnclosingschemas().stream().filter(es -> es.getEnclosingSchema().getName().equals("GithubwfRootOnOneOf2Workflow_dispatchInputsPatternProperties0AllOf0IfType")).findFirst().get();
	...
	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.GithubwfRootOnOneOf2Workflow_dispatchInputsPatternProperties0AllOf0IfGrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.githubwfrootononeof2workflow_dispatchinputspatternproperties0allof0if");
