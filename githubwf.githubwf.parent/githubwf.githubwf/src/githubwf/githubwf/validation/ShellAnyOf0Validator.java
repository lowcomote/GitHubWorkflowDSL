...
public class ShellAnyOf0Validator extends AbstractShellAnyOf0Validator {
	
	@Inject githubwf.githubwf.services.ShellAnyOf0GrammarAccess grammarAccess;	
	...
	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.ShellAnyOf0GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.shellanyof0");
