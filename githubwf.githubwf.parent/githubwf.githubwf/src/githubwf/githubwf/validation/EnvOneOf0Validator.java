...
public class EnvOneOf0Validator extends AbstractEnvOneOf0Validator {
	
	@Inject githubwf.githubwf.services.EnvOneOf0GrammarAccess grammarAccess;	
	
	private List<EObject> processedPatternProperties = new ArrayList<EObject>();

	@Check
	public void oneOfValidationEnvOneOf0AdditionalProperties(githubwf.EnvOneOf0AdditionalProperties enclosingEClass){
	//	String enclosingEClassText = NodeModelUtils.getTokenText(NodeModelUtils.getNode(enclosingEClass));
		...
		Resource resource0 = reset0.createResource(URI.createURI("platform:/dummy.envoneof0additionalpropertiesoneof0"));
		try {
		...
		Resource resource1 = reset1.createResource(URI.createURI("platform:/dummy.envoneof0additionalpropertiesoneof1"));
		try {
		...
		Resource resource2 = reset2.createResource(URI.createURI("platform:/dummy.envoneof0additionalpropertiesoneof2"));
		try {
		...
		if(validSchemasCount==0){
			error("No valid schema found in the OneOf EnvOneOf0AdditionalProperties", null);
		}else if(validSchemasCount>1){
			error("Found "+validSchemasCount+" valid schemas in the OneOf EnvOneOf0AdditionalProperties", null);
		}	
	}

	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.EnvOneOf0GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.envoneof0");
