...
public class ContainerPortsItemsOneOf1Validator extends AbstractContainerPortsItemsOneOf1Validator {
	
	@Inject githubwf.githubwf.services.ContainerPortsItemsOneOf1GrammarAccess grammarAccess;	
	...
	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.ContainerPortsItemsOneOf1GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.containerportsitemsoneof1");
