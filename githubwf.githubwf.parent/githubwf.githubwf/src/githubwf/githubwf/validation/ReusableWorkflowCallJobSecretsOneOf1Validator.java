...
public class ReusableWorkflowCallJobSecretsOneOf1Validator extends AbstractReusableWorkflowCallJobSecretsOneOf1Validator {
	
	@Inject githubwf.githubwf.services.ReusableWorkflowCallJobSecretsOneOf1GrammarAccess grammarAccess;	
	
	private List<EObject> processedPatternProperties = new ArrayList<EObject>();

	@Check
	public void enumValidationReusableWorkflowCallJobSecretsOneOf1(githubwf.ReusableWorkflowCallJobSecretsOneOf1 enclosingEClass){
		...
		EnclosingSchema enclosingSchema = relatedSchemas.getEnclosingschemas().stream().filter(es -> es.getEnclosingSchema().getName().equals("ReusableWorkflowCallJobSecretsOneOf1")).findFirst().get();
		List<JsonDocument> enumJsonDocuments=enclosingSchema.getConstEnum().getEnum().getEnum();
		JsonDocument enumJson = enclosingEClass.getSecrets();
	...
	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.ReusableWorkflowCallJobSecretsOneOf1GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.reusableworkflowcalljobsecretsoneof1");
