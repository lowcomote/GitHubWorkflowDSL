...
public class ConfigurationOneOf2Validator extends AbstractConfigurationOneOf2Validator {
	
	@Inject githubwf.githubwf.services.ConfigurationOneOf2GrammarAccess grammarAccess;	
	...
	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.ConfigurationOneOf2GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.configurationoneof2");
