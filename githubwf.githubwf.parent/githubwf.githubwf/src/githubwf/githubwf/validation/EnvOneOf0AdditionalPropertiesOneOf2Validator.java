...
public class EnvOneOf0AdditionalPropertiesOneOf2Validator extends AbstractEnvOneOf0AdditionalPropertiesOneOf2Validator {
	
	@Inject githubwf.githubwf.services.EnvOneOf0AdditionalPropertiesOneOf2GrammarAccess grammarAccess;	
	...
	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.EnvOneOf0AdditionalPropertiesOneOf2GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.envoneof0additionalpropertiesoneof2");
