...
public class ConfigurationOneOf4Validator extends AbstractConfigurationOneOf4Validator {
	
	@Inject githubwf.githubwf.services.ConfigurationOneOf4GrammarAccess grammarAccess;	
	...
	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.ConfigurationOneOf4GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.configurationoneof4");
