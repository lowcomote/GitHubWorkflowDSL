...
public class RefOneOf0AllOf2NotValidator extends AbstractRefOneOf0AllOf2NotValidator {
	
	@Inject githubwf.githubwf.services.RefOneOf0AllOf2NotGrammarAccess grammarAccess;	
	...
	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.RefOneOf0AllOf2NotGrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.refoneof0allof2not");
