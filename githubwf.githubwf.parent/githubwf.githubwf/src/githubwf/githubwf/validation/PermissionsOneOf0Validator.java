...
public class PermissionsOneOf0Validator extends AbstractPermissionsOneOf0Validator {
	
	@Inject githubwf.githubwf.services.PermissionsOneOf0GrammarAccess grammarAccess;	
	
	private List<EObject> processedPatternProperties = new ArrayList<EObject>();

	@Check
	public void enumValidationPermissionsOneOf0(githubwf.PermissionsOneOf0 enclosingEClass){
		...
		EnclosingSchema enclosingSchema = relatedSchemas.getEnclosingschemas().stream().filter(es -> es.getEnclosingSchema().getName().equals("PermissionsOneOf0")).findFirst().get();
		List<JsonDocument> enumJsonDocuments=enclosingSchema.getConstEnum().getEnum().getEnum();
		JsonDocument enumJson = enclosingEClass.getPermissions();
	...
	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.PermissionsOneOf0GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.permissionsoneof0");
