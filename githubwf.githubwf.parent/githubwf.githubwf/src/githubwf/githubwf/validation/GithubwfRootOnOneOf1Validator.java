...
public class GithubwfRootOnOneOf1Validator extends AbstractGithubwfRootOnOneOf1Validator {
	
	@Inject githubwf.githubwf.services.GithubwfRootOnOneOf1GrammarAccess grammarAccess;	
	
	private List<EObject> processedPatternProperties = new ArrayList<EObject>();

	@Check
	public void enumValidationEvent(githubwf.Event enclosingEClass){
		...
		EnclosingSchema enclosingSchema = relatedSchemas.getEnclosingschemas().stream().filter(es -> es.getEnclosingSchema().getName().equals("Event")).findFirst().get();
		List<JsonDocument> enumJsonDocuments=enclosingSchema.getConstEnum().getEnum().getEnum();
		JsonDocument enumJson = enclosingEClass.getEvent();
	...
	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.GithubwfRootOnOneOf1GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.githubwfrootononeof1");
