...
public class RefOneOf0AllOf1Validator extends AbstractRefOneOf0AllOf1Validator {
	
	@Inject githubwf.githubwf.services.RefOneOf0AllOf1GrammarAccess grammarAccess;	
	
	private List<EObject> processedPatternProperties = new ArrayList<EObject>();

	@Check
	public void notValidationRefOneOf0AllOf1(githubwf.RefOneOf0AllOf1 enclosingEClass){
	//	String enclosingEClassText = NodeModelUtils.getTokenText(NodeModelUtils.getNode(enclosingEClass));
		...
		Resource resource = reset.createResource(URI.createURI("platform:/dummy.refoneof0allof1not"));
		...
		if(isValid){
			error("The Not RefOneOf0AllOf1 schema must not be valid", null);
		}	
	}

	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.RefOneOf0AllOf1GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.refoneof0allof1");
