...
public class NormalJobStepsItemsAllOf0OneOf1Validator extends AbstractNormalJobStepsItemsAllOf0OneOf1Validator {
	
	@Inject githubwf.githubwf.services.NormalJobStepsItemsAllOf0OneOf1GrammarAccess grammarAccess;	
	...
	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.NormalJobStepsItemsAllOf0OneOf1GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.normaljobstepsitemsallof0oneof1");
