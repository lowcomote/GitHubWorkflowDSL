...
public class RefOneOf0AllOf0Validator extends AbstractRefOneOf0AllOf0Validator {
	
	@Inject githubwf.githubwf.services.RefOneOf0AllOf0GrammarAccess grammarAccess;	
	
	private List<EObject> processedPatternProperties = new ArrayList<EObject>();

	@Check
	public void notValidationRefOneOf0AllOf0(githubwf.RefOneOf0AllOf0 enclosingEClass){
	//	String enclosingEClassText = NodeModelUtils.getTokenText(NodeModelUtils.getNode(enclosingEClass));
		...
		Resource resource = reset.createResource(URI.createURI("platform:/dummy.refoneof0allof0not"));
		...
		if(isValid){
			error("The Not RefOneOf0AllOf0 schema must not be valid", null);
		}	
	}

	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.RefOneOf0AllOf0GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.refoneof0allof0");
