...
public class NormalJobRuns_45onOneOf1AnyOf4Validator extends AbstractNormalJobRuns_45onOneOf1AnyOf4Validator {
	
	@Inject githubwf.githubwf.services.NormalJobRuns_45onOneOf1AnyOf4GrammarAccess grammarAccess;	
	
	private List<EObject> processedPatternProperties = new ArrayList<EObject>();

	@Check
	public void constValidationNormalJobRuns_45onOneOf1AnyOf4Items0(githubwf.NormalJobRuns_45onOneOf1AnyOf4Items0 enclosingEClass){
		...
		EnclosingSchema enclosingSchema = relatedSchemas.getEnclosingschemas().stream().filter(es -> es.getEnclosingSchema().getName().equals("NormalJobRuns_45onOneOf1AnyOf4Items0")).findFirst().get();
		JsonDocument constJsonDocument=enclosingSchema.getConstEnum().getConst().getConst();
		JsonDocument inputJsonDocument = enclosingEClass.getItems0();
	...
	@Check
	public void enumValidationArchitecture(githubwf.Architecture enclosingEClass){
		ResourceSet reset  = ConverterUtil.createResourceSet();
		Resource resource = reset.getResource(URI.createPlatformResourceURI("/githubwf/model/githubwf.relatedSchemas", true), true);
		...
		RelatedSchemas relatedSchemas = (RelatedSchemas )resource.getContents().get(0);
		EnclosingSchema enclosingSchema = relatedSchemas.getEnclosingschemas().stream().filter(es -> es.getEnclosingSchema().getName().equals("Architecture")).findFirst().get();
		List<JsonDocument> enumJsonDocuments=enclosingSchema.getConstEnum().getEnum().getEnum();
		JsonDocument enumJson = enclosingEClass.getArchitecture();
	...
	@Check
	public void enumValidationMachine(githubwf.Machine enclosingEClass){
		ResourceSet reset  = ConverterUtil.createResourceSet();
		Resource resource = reset.getResource(URI.createPlatformResourceURI("/githubwf/model/githubwf.relatedSchemas", true), true);
		...
		RelatedSchemas relatedSchemas = (RelatedSchemas )resource.getContents().get(0);
		EnclosingSchema enclosingSchema = relatedSchemas.getEnclosingschemas().stream().filter(es -> es.getEnclosingSchema().getName().equals("Machine")).findFirst().get();
		List<JsonDocument> enumJsonDocuments=enclosingSchema.getConstEnum().getEnum().getEnum();
		JsonDocument enumJson = enclosingEClass.getMachine();
	...
	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.NormalJobRuns_45onOneOf1AnyOf4GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.normaljobruns_45ononeof1anyof4");
