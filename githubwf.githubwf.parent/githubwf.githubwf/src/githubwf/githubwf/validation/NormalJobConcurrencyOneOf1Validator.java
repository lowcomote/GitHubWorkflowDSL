...
public class NormalJobConcurrencyOneOf1Validator extends AbstractNormalJobConcurrencyOneOf1Validator {
	
	@Inject githubwf.githubwf.services.NormalJobConcurrencyOneOf1GrammarAccess grammarAccess;	
	...
	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.NormalJobConcurrencyOneOf1GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.normaljobconcurrencyoneof1");
