...
public class GithubwfRootOnOneOf2Workflow_dispatchInputsPatternProperties0AllOf1Validator extends AbstractGithubwfRootOnOneOf2Workflow_dispatchInputsPatternProperties0AllOf1Validator {
	
	@Inject githubwf.githubwf.services.GithubwfRootOnOneOf2Workflow_dispatchInputsPatternProperties0AllOf1GrammarAccess grammarAccess;	
	
	private List<EObject> processedPatternProperties = new ArrayList<EObject>();

	@Check
	public void ifThenElseValidationGithubwfRootOnOneOf2Workflow_dispatchInputsPatternProperties0AllOf1(githubwf.GithubwfRootOnOneOf2Workflow_dispatchInputsPatternProperties0AllOf1 enclosingEClass){
		...
		Resource resourceIf = resetIf.createResource(URI.createURI("platform:/dummy.githubwfrootononeof2workflow_dispatchinputspatternproperties0allof1if"));
		...
		if(isIfSchemaValid){
			...
			Resource resourceThen = resetThen.createResource(URI.createURI("platform:/dummy.githubwfrootononeof2workflow_dispatchinputspatternproperties0allof1then"));
			try {
			...
			if(!isThenValid){
				error("The IF GithubwfRootOnOneOf2Workflow_dispatchInputsPatternProperties0AllOf1 Schema is valid but the THEN GithubwfRootOnOneOf2Workflow_dispatchInputsPatternProperties0AllOf1 is not valid", null);
			}
		}else{
		}
	}
	...
	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.GithubwfRootOnOneOf2Workflow_dispatchInputsPatternProperties0AllOf1GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.githubwfrootononeof2workflow_dispatchinputspatternproperties0allof1");
