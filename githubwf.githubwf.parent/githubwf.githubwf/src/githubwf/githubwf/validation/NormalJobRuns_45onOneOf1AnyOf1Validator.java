...
public class NormalJobRuns_45onOneOf1AnyOf1Validator extends AbstractNormalJobRuns_45onOneOf1AnyOf1Validator {
	
	@Inject githubwf.githubwf.services.NormalJobRuns_45onOneOf1AnyOf1GrammarAccess grammarAccess;	
	
	private List<EObject> processedPatternProperties = new ArrayList<EObject>();

	@Check
	public void constValidationNormalJobRuns_45onOneOf1AnyOf1Items0(githubwf.NormalJobRuns_45onOneOf1AnyOf1Items0 enclosingEClass){
		...
		EnclosingSchema enclosingSchema = relatedSchemas.getEnclosingschemas().stream().filter(es -> es.getEnclosingSchema().getName().equals("NormalJobRuns_45onOneOf1AnyOf1Items0")).findFirst().get();
	...
	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.NormalJobRuns_45onOneOf1AnyOf1GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.normaljobruns_45ononeof1anyof1");
