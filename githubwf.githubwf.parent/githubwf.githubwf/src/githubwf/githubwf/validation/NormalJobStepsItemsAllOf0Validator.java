...
public class NormalJobStepsItemsAllOf0Validator extends AbstractNormalJobStepsItemsAllOf0Validator {
	
	@Inject githubwf.githubwf.services.NormalJobStepsItemsAllOf0GrammarAccess grammarAccess;	
	
	private List<EObject> processedPatternProperties = new ArrayList<EObject>();

	@Check
	public void oneOfValidationNormalJobStepsItemsAllOf0(githubwf.NormalJobStepsItemsAllOf0 enclosingEClass){
	//	String enclosingEClassText = NodeModelUtils.getTokenText(NodeModelUtils.getNode(enclosingEClass));
		...
		Resource resource0 = reset0.createResource(URI.createURI("platform:/dummy.normaljobstepsitemsallof0oneof0"));
		try {
		...
		Resource resource1 = reset1.createResource(URI.createURI("platform:/dummy.normaljobstepsitemsallof0oneof1"));
		try {
		...
		if(validSchemasCount==0){
			error("No valid schema found in the OneOf NormalJobStepsItemsAllOf0", null);
		}else if(validSchemasCount>1){
			error("Found "+validSchemasCount+" valid schemas in the OneOf NormalJobStepsItemsAllOf0", null);
		}	
	}

	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.NormalJobStepsItemsAllOf0GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.normaljobstepsitemsallof0");
