...
public class NormalJobContinue_45on_45errorOneOf1Validator extends AbstractNormalJobContinue_45on_45errorOneOf1Validator {
	
	@Inject githubwf.githubwf.services.NormalJobContinue_45on_45errorOneOf1GrammarAccess grammarAccess;	
	...
	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.NormalJobContinue_45on_45errorOneOf1GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.normaljobcontinue_45on_45erroroneof1");
