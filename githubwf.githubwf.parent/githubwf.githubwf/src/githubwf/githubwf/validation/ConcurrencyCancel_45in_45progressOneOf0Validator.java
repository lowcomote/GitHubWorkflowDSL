...
public class ConcurrencyCancel_45in_45progressOneOf0Validator extends AbstractConcurrencyCancel_45in_45progressOneOf0Validator {
	
	@Inject githubwf.githubwf.services.ConcurrencyCancel_45in_45progressOneOf0GrammarAccess grammarAccess;	
	...
	private void independentValidation(String eClassName, List<EObject> processedListName, InputStream inputStream){
		try {
			Method getRule = githubwf.githubwf.services.ConcurrencyCancel_45in_45progressOneOf0GrammarAccess.class.getDeclaredMethod("get"+eClassName+"Rule");
			...
			Thread validationThread = new Thread(new Runnable() {
			    ...
			    public void run() {
			    	try {
						URI uri=URI.createURI("platform:/dummy.concurrencycancel_45in_45progressoneof0");
